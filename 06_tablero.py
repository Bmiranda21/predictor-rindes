...
with tab2:
    ...
    Para este tablero, se integró el archivo **`adrs.geojson`**. A diferencia de los límites políticos tradicionales (Partidos), este archivo contiene las **geometrías propias de la empresa**, permitiendo visualizar exactamente las áreas de influencia (ADRs) que el equipo técnico delimitó.
...
<div style='text-align:center; font-size:11px; color:#374151; padding: 8px 0;'>
    Predictor de Rinde Ceres Tolvas &nbsp;·&nbsp; GIS-Enabled Dashboard &nbsp;·&nbsp; {datetime.now().strftime("%d/%m/%Y")}
</div>
""", unsafe_allow_html=True)